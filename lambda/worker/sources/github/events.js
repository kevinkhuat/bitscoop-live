			...
			switch(item.type) {
				...
				case 'PushEvent':
					for (let j = 0; j < item.payload.commits.length; j++) {
						...
						messageTags = commit.message.match(tagRegex);
				...
				case 'IssuesEvent':
					...
					newTags = [];
					titleTags = issue.title.match(tagRegex);
					...
					bodyTags = issue.body.match(tagRegex);
					...
					if (issue.user.login !== this.connection.metadata.login) {
						newContact = {
				...
				case 'IssueCommentEvent':
					...
					newTags = [];
					bodyTags = comment.body.match(tagRegex);
