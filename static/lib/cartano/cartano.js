define('cartano', ['deferred-ap', 'jquery', 'leaflet', 'leaflet-draw', 'leaflet-draw-drag', 'leaflet-featuregroup-subgroup', 'leaflet-markercluster', 'mapbox', 'promises-ap'], function() {
var args = arguments;
var cache = {'deferred-ap':args[0],'jquery':args[1],'leaflet':args[2],'leaflet-draw':args[3],'leaflet-draw-drag':args[4],'leaflet-featuregroup-subgroup':args[5],'leaflet-markercluster':args[6],'mapbox':args[7],'promises-ap':args[8]};
var mappings = {};
...
_define('src/cartano', function(require, module, exports) {
...
function Map(id, options) {
	var controls, drawStart, drawEnd, element, estimatedLayerControl, features, geofilters, layers, map, markers, self = this;

	estimatedLayerControl  = leaflet.Control.Layers.extend({
		_onInputClick: function(Layer, name) {
			var innerSelf = this;
			//Trigger a click event on the map to de-spiderfy any clusterGroups that are spiderfied.
			self.object.fireEvent('click');
			setTimeout(function() {
				L.Control.Layers.prototype._onInputClick.call(innerSelf, Layer, name);
			}, 200);
		}
	});
	...
	layers.path = new leaflet.FeatureGroup().addTo(map);
	...
	if (options.layerControl === true) {
		controls.layer = new estimatedLayerControl(null, {
...
Map.prototype = {
	...
	addData: function Map$addData(data, callback) {
		var i, deferred, estimatedMarkers, marker, validMarkers;
		...
		for (i in estimatedMarkers) {
			this.layers.markers.estimated.addLayer(estimatedMarkers[i]);
		}

		this.markers.addLayers(validMarkers);
