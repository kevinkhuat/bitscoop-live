...
module.exports = function(data) {
	...
	if (data && data.length > 0 && data[0].upload_history.length > 0) {
		objectCache = {
			tags: {}
		};

		content = new Array(data[0].upload_history.length);
		tags = [];
		events = new Array(data[0].upload_history.length);

		for (let i = 0; i < data[0].upload_history.length; i++) {
			let item = data[0].upload_history[i];
			let newTags = [];
			...
			let newEvent = {
				...
				identifier: this.connection._id.toString('hex') + ':::created:::youtube:::' + item.id,
