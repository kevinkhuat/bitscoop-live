...
module.exports = function(req, res, next) {
	...
	// Spoof user object
	if (config.spoof && config.spoof.user) {
		req.user = config.spoof.user;
		console.log(config.spoof.user);
	}
	// Spoof session
	if (config.spoof && config.spoof.session) {
		req.session = config.spoof.session;
	}
