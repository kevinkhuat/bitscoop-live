...
function create(req, res, next) {
	if (req.user && req.session && req.session.csrf_secret) {
		let valid = tokens.verify(req.session.csrf_secret, req.cookies[config.csrf.cookieName]);
		let token = tokens.create(req.session.csrf_secret);

		if (!req.cookies[config.csrf.cookieName] || !valid) {
