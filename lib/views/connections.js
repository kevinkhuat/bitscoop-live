...
complete.post(function(req, res, next) {
	...
	validate('#/types/webhook-events', req.body.event)
		.catch(function() {
			return Promise.reject(httpErrors(400));
		})
		.then(function() {
			return Promise.all([
				validate('#/types/uuid4', req.body.connection_id),
				validate('#/types/uuid4', req.body.provider_id)
			])
				.catch(function() {
					return Promise.reject(httpErrors(404));
				});
		...
		.then(function(connection) {
			return mongo.db('live').collection('providers').findOne({
				...
				.then(function(provider) {
					...
					let path = 'providers/' + provider.remote_map_id.toString('hex');
...
connections.post(loginRequired(404), csrf.validate, function(req, res, next) {
	...
	Promise.all([
		...
		.then(function(provider) {
			...
			let path = 'providers/' + provider.remote_map_id.toString('hex') + '/connections';
			let body = {
				name: name,
				endpoints: _.uniq(endpoints)
			};
