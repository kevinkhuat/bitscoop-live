...
const get = require('explorer/lib/views/api/templates/get');
...
const sortDictionary = require('explorer/lib/util/sort-dictionary');
...
search.delete(loginRequired(404), csrf.validate, function(req, res, next) {
	...
	return validate(hexId, '/types/uuid4')
		...
		.then(function() {
			return mongo.db('explorer').collection('searches').deleteOne({
				_id: gid(hexId),
...
search.patch(loginRequired(404), csrf.validate, function(req, res, next) {
	...
	return validate(hexId, '/types/uuid4')
		...
		.then(function() {
			return validate(body, '/objects/search');
		})
		.catch(function() {
		...
		.then(function() {
			...
			return mongo.db('explorer').collection('searches').updateOne(filter, {
				$set: body
			})
				.then(function(data) {
					if (data.result.n === 0) {
					...
					return mongo.db('explorer').collection('searches').findOne(filter);
				});
		})
...
searches.get(loginRequired(404), function(req, res, next) {
	...
	let validation = validate(pagination, '/requests/get')
	...
	return validation
		.then(function(pagination) {
			...
			let count = mongo.db('explorer').collection('searches').count(filter);
			let result = mongo.db('explorer').collection('searches').find(filter, {
		...
		.spread(function(data, count) {
			...
			let pagination = validation.value();
...
searches.post(loginRequired(404), csrf.validate, function(req, res, next) {
	...
	return validate(body, '/objects/search')
		...
		.then(function() {
			...
			return mongo.db('explorer').collection('searches').update({
				hash: hash,
				user_id: req.user._id
			}, {
				$set: {
				...
				.then(function() {
					return mongo.db('explorer').collection('searches').findOne({
						hash: hash
					});
				});
		})
