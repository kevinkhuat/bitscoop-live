	...
	return validate(hexId, '/types/uuid4')
		...
		.then(function() {
			let promises = _.map(req.body.tags, function(tag) {
				return mongo.db('explorer').collection('tags').update({
					tag: tag,
		...
		.then(function() {
			...
			return mongo.db('explorer').collection(type).updateOne(filter, {
				$addToSet: {
					'tagMasks.added': {
						$each: req.body.tags
					}
				...
				.then(function(data) {
					...
					return mongo.db('explorer').collection(type).findOne(filter);
				});
		})
...
function hydrateEvent(document, userId) {
	...
	let promises = _.map(subtypes, function(type) {
		if (type === 'locations' && document.location) {
			return mongo.db('explorer').collection(type).findOne({
